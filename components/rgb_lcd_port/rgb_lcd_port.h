 ...
 *
 * A reusable internal buffer sized for `EXAMPLE_LCD_H_RES * EXAMPLE_LCD_V_RES * 2`
 * bytes is used to hold the cropped data. The maximum required capacity for
 * a given call equals `(Xend - Xstart) * (Yend - Ystart) * 2` bytes (two bytes
 * per pixel).
 *
